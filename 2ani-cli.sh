...
help_info() {
    ...
    Options:
      ...
      --dub
        Play dubbed version
      -L, --language 
        Specify the language the result should be (jp,jl), where jp is pure japanese, jl for japanese in english
      --rofi
...
search_anime() {
    language="${2:-englishName}"
    [[ "$language" == "jp" ]] && language="nativeName"
    [[ "$language" == "jl" ]] && language="name"

    search_gql="query(        \$search: SearchInput        \$limit: Int        \$page: Int        \$translationType: VaildTranslationTypeEnumType        \$countryOrigin: VaildCountryOriginEnumType    ) {    shows(        search: \$search        limit: \$limit        page: \$page        translationType: \$translationType        countryOrigin: \$countryOrigin    ) {        edges {            _id $language availableEpisodes __typename       }    }}"
...
| g' | sed -nE "s|.*_id\":\"([^\"]*)\",\"$language\":\"(.+)\",.*${mode}\":([1-9][^,]*).*|\1	\2 (\3 episodes)|p" | sed 's/\\"//g'
}
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -L | --language)
            lang="$2"
            [ $# -lt 2 ] && die "Expecting 1 argument: specify language (e.g., 'jp' or 'jl')."
            [[ "$lang" == "jp" || "$lang" == "jl" ]] || die "Invalid language! Use 'jp' or 'jl'."
            shift
            ;;
...
case "$search" in
    ...
    *)
        ...
        anime_list=$(search_anime "$query" "$lang")
